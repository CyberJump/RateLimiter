...
| :--- | :--- | :--- |
| **Cold-Start Warm-up** | ${result.coldStartLatencyMs ? result.coldStartLatencyMs + ' ms' : 'N/A'} | Pre-test HTTP warm-up probe |
| **Average** | ${latency.avgLatencyMs} ms | k6 measured (steady-state) |
...
\`\`\`
- Tested Gateway Throughput:     ${(system.gatewayProcessingThroughputRps || traffic.generatedRps || 0).toLocaleString()} req/s
- P95 Request Processing Latency: ${(latency?.p95LatencyMs ?? 0)} ms
- P99 Request Processing Latency: ${(latency?.p99LatencyMs ?? 0)} ms
- Estimated Redis Ops Throughput: ${(redis?.redisOpsPerSec ?? 0).toLocaleString()} ops/sec (derived)
- Tested Concurrency (VUs):       ${scenario.vus} VUs
- Algorithm Validation Accuracy:  ${(validation?.accuracy ?? 0)}%
\`\`\`
...
`;
  ...
  generateConsoleSummary(results: DetailedBenchmarkResult[], resume: ResumeMetrics): string {
    ...
    results.forEach((r, index) => {
      ...
      output += `  Latency:   Avg: ${latency?.avgLatencyMs ?? 'N/A'}ms | P50: ${latency?.p50LatencyMs ?? 'N/A'}ms | P90: ${latency?.p90LatencyMs ?? 'N/A'}ms | P95: ${latency?.p95LatencyMs ?? 'N/A'}ms | P99: ${latency?.p99LatencyMs ?? 'N/A'}ms${r.coldStartLatencyMs ? ` | ColdStart: ${r.coldStartLatencyMs}ms` : ''}\n`;
